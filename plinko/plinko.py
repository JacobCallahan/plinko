...
from plinko import core, helpers
...
def plinko(clix_diff, apix_diff, pytest_export, allow_dupes, debug):
    if debug:
    ...
    if clix_diff:
        ...
        if res:
            clix_pt = core.plinko_to_ptcommand(res["found"], allow_dupes)
            if clix_pt:
    ...
    if apix_diff:
        ...
        if res:
            apix_pt = core.plinko_to_ptcommand(res["found"], allow_dupes)
            if apix_pt:
